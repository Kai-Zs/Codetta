<template>
  <div class="flex flex-col gap-4">
    <template v-if="!revealed">
      <p class="text-sm text-gray-500 text-center py-8">仔细阅读题目，思考解题思路</p>
    ...
    <template v-else>
      ...
      <p class="text-xs text-gray-400 text-center">人工智能生成，仅供参考</p>
      <div class="flex gap-3 mt-2">
        <button @click="selfEval = 'correct'" :class="selfEval === 'correct' ? 'bg-green text-white' : 'border border-gray-200 text-gray-600'"
          class="flex-1 py-3 rounded-xl text-sm font-medium transition">做对了</button>
        <button @click="selfEval = 'wrong'" :class="selfEval === 'wrong' ? 'bg-red-400 text-white' : 'border border-gray-200 text-gray-600'"
